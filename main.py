...
def task_3():
    duration_s, sample_rate_hz, signal_amp, signal_freq = 10, 128, 1, 50
    noise_std = 0.01
    ...
    plt.subplot(3, 2, 1)
    plt.plot(time_s, signal)
    plt.title('Time Domain: 50 Hz Noise')
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude (V)')
    plt.grid(True)
    ...
    plt.subplot(3, 2, 2)
    plt.plot(fft_freqs[:len(fft_freqs)//2], signal_amps[:len(signal_amps)//2])
    plt.title('Amplitude Spectrum: 50 Hz Noise')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Amplitude')
    plt.grid(True)
    ...
    plt.subplot(3, 2, 4)
    plt.plot(fft_freqs[:len(fft_freqs)//2], noise_amps[:len(noise_amps)//2])
    plt.title('Amplitude Spectrum: Signal')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Amplitude')
    plt.grid(True)
    ...
    plt.savefig("task_3_signals.png")
    plt.tight_layout()
    plt.show()

    title, (wp, ws) = "bandstop", ((48.8, 50.6*1.007), (48.8*1.007, 50.6))
    ...
    plt.subplot(2, 1, 2)
    plt.plot(f_resp_freqs_hz, np.abs(filter_response))
    plt.title(f'Frequency Response of the butterworth Filter ({title})')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Gain')
    plt.grid(True)

    plt.savefig(f"task_3_filter_{title}.png")
    plt.tight_layout()
    plt.show()
    ...
    compute_snr(noise, signal, "Original")
    compute_snr(lfilter(b, a, noise), lfilter(b, a, signal), "Filtered")
...
def task_4():
    for (filename, ssss) in [
        ("audio_8k.wav", [
            (("lowpass", "lt450"), (450, 450*1.1)),
            (("bandpass", "450_1000"), ((450*1.1, 1000), (450, 1000*1.1))),
            (("bandpass", "1000_4000"), ((1000*1.05, 3900), (1000, 3900*1.05))),
        ]), ("audio_44k.wav", [
            (("lowpass", "lt450"), (450, 450*1.1)),
            (("bandpass", "450_1000"), ((450*1.1, 1000), (450, 1000*1.1))),
            (("bandpass", "1000_4000"), ((1000*1.15, 4000), (1000, 4000*1.15))),
        ])
    ]:
        signal, sample_rate = sf.read(filename)
        time_s = np.arange(len(signal)) / sample_rate
        fft_freqs = fftfreq(time_s.shape[0], d=1/sample_rate)
        ...
        plt.savefig(f"task_4_signal_{filename[:-4]}.png")
        plt.tight_layout()
        plt.show()

        for (title, p), (wp, ws) in ssss:
            # if "1000_4000" == p and filename == "audio_8k.wav":
            #     continue
            print(f"\n\n\nFilter: {title}")
            print(f"wp {wp}, ws: {ws}")
            lowest_butterworth_order, butterworth_natural_frequency = cheb1ord(
                wp=wp,         # cut-off freq
                ws=ws,         # stopband freq
                gpass=3,
                gstop=20,
            ...
            wavfile.write(f"out_{title}_{p}_{filename}", sample_rate, filtered_signal)

            plt.subplot(2, 2, 1)
            plt.plot(time_s, filtered_signal)
            plt.title(f'Time Domain: Filtered Signal ({title} {p})')
            plt.xlabel('Time (s)')
            plt.ylabel('Amplitude (V)')
            plt.grid(True)
            ...
            plt.subplot(2, 2, 2)
            plt.plot(fft_freqs[:len(fft_freqs)//2], filtered_signal_amps[:len(filtered_signal_amps)//2])
            plt.title(f'Amplitude Spectrum: Filtered Signal ({title} {p})')
            plt.xlabel('Frequency (Hz)')
            plt.ylabel('Amplitude')
            plt.grid(True)
            ...
            f_resp_freqs_rad_p_s, filter_response = freqz(b, a, worN=8000)
            # convert to Hz
            f_resp_freqs_hz = f_resp_freqs_rad_p_s * sample_rate / (2 * np.pi)

            plt.subplot(2, 1, 2)
            plt.plot(f_resp_freqs_hz, np.abs(filter_response))
            plt.title(f'Frequency Response of the chebyshev Filter ({title} {p})')
            plt.xlabel('Frequency (Hz)')
            plt.ylabel('Gain')
            plt.grid(True)

            plt.savefig(f"task_4_{filename[:-4]}_filter_{title}_{p}.png")
            plt.tight_layout()
            plt.show()
